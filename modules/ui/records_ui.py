...
def dialog_confirmar_registro(record, jugadora):
    ...
    with col3:
        if st.button(t(":material/check: Confirmar"), type="primary"):
            success = upsert_record_db(record)
...
def records_form(jugadora):
    ...
    # ---------------------------------------
    record, is_valid, validation_msg = record_form(record, jugadora["genero"])

    # ---------------------------------------
    # 5. Procesamiento del guardado
    # ---------------------------------------
    #st.dataframe(jugadora)
    if st.button(f":material/save: {t('Guardar')}", key="btn_reg_template"):


        if not is_valid:
            st.warning(validation_msg)
        else:
            dialog_confirmar_registro(record, jugadora)
